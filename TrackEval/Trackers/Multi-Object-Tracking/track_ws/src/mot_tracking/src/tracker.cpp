...
void clusterq(const cv::Mat& cost_matrix, std::vector<std::vector<int> >& cltrack, std::vector<std::vector<int> >& dettrack){
	...
	while(total_tracks<num_tracks){
		...
		if(TEST)for(auto& t:potential_track){
			if(TEST)std::cout<<"potential_track "<<t<<std::endl;}
		...
		if(TEST)std::cout<<visited_tracks.empty()<<std::endl;

		std::vector<int> visited_tracksv;
		for(auto& x:visited_tracks){
			visited_tracksv.push_back(x);
			if(TEST)std::cout<<"visited_tracks "<<x<<std::endl;
		}
		if(TEST)std::cout<<visited_detections.empty()<<std::endl;

		std::vector<int> visited_detectionsv;
		for(auto& x:visited_detections){
			visited_detectionsv.push_back(x);
			if(TEST)std::cout<<"visited_detections "<<x<<std::endl;
		}
		...
		if(TEST)std::cout<<"total_tracks "<<total_tracks<<std::endl;
...
void Tracker::Init(const Detection& detections, float& time){
	...
	if(!init_){//表示第一帧存下的检测结果
		if(TEST)std::cout<<"#################### TRACKER INIT ####################"<<std::endl;
		...
		if(TEST)std::cout<<"#################### TRACKER start_tracking_ INIT ####################"<<std::endl;
...
void Tracker::track(const Detection& detections,float& time, std::vector<Eigen::VectorXd>& result){
	...
	if(!init_ || !start_tracking_){
		...
		if(TEST)std::cout<<"#################### TRACKER tracking #####################"<<std::endl;
		not_associated_.clear();
		not_associated_.swap(not_associated_);
		//级联关联
		if(TEST)std::cout<<"tracksstart "<<tracks_.size()<<" confirmed_tracks_ "<<confirmed_tracks_.size()<<std::endl;
		...
		if(TEST)std::cout<<"confirmed_tracks_ "<<confirmed_tracks_.size()<<" "<<"unconfirmed_tracks_ "<<unconfirmed_tracks_.size()<<std::endl;
		tracks_.clear();
		...
		if(q.total()==0){
			...
			for(int i =0; i<clustersize; ++i){
				...
				if(TEST)std::cout<<"###############q \n"<<q<<"\n"<<std::endl;
				if(TEST)std::cout<<"colsize "<<colsize <<" rowsize "<<rowsize<<std::endl;
				...
				for(int row=0; row<rowsize; ++row){
					...
					for(int col=1; col<x.cols; ++col){
						if(TEST)std::cout<<"col "<<cltrack[i][col-1]<<std::endl;
						if(TEST)std::cout<<"row "<<rowinq<<" "<<std::endl;
						if(TEST)std::cout<<q.at<int>(rowinq,cltrack[i][col-1])<<std::endl;
						x.at<int>(row,col) = q.at<int>(rowinq, cltrack[i][col-1]);
					}
				}

				//if(TEST)q(cv::Rect(colmin, rowmin, colsize, rowsize)).copyTo(x(cv::Rect(1, 0, colsize, rowsize)));
		...
		if(q.total()==0){
			...
			if(TEST)std::cout<<"########## BETA ##########\n"<<beta<<std::endl;

			int i=0;
			for(const auto& track:confirmed_tracks_){
		    	if(TEST)std::cout<<"MeaisAsso "<<MeaisAsso[i]<<std::endl;
				if(MeaisAsso[i]){
					std::vector<Eigen::VectorXd> Zv;
		...
		for(auto& tr:tracks_){
			...
			if(TEST)std::cout<<"track->Age() "<<tr->Age()<<std::endl;
			if(tr->Age()<3)
		...
		if(unconfirmed_tracks_.size()>0){
			...
			if(TEST)std::cout<<not_associated_.size()<<std::endl;
			...
			not_associated_.clear();
			not_associated_.swap(not_associated_);
			if(TEST)std::cout<<"######## not asso clear ###############"<<not_associated_.size()<<std::endl;

			for(const auto& track:unconfirmed_tracks_){
				if(TEST)std::cout<<"########################## unconfirm preditcion"<< track->GetId() <<" ########################"<<std::endl;
				track->Prediction(time);
				if(TEST)std::cout<<"####"<< std::flush;
				if(TEST)std::cout<<"########################## unconfirm preditcion done"<< track->GetId() <<" ########################"<<std::endl;
			}

			if(TEST)std::cout<<"1_"<<std::endl;
			...
			if(TEST)std::cout<<"##########  second costMat #####\n"<<costMat<<std::endl;
...
//std::cout<<"1"<<std::endl;
		...
		if(TEST)std::cout<<"######## not asso ###############"<<not_associated_.size()<<std::endl;
		/*for(auto& track:confirmed_tracks_){
			tracks_.push_back(track);
		...
		if(TEST)std::cout<<"tracks "<<tracks_.size()<<std::endl;
		...
		delete_tracks();
		manage_tracks(time);
		if(TEST)std::cout<<"tracks2 "<<tracks_.size()<<std::endl;
...
{	
	...
	if(TEST)std::cout<<"#################### TRACKER ASSOCISTATE #########################"<<std::endl;
	...
	if(TEST)std::cout<<"######## assosiate not asso ###############"<<not_associated_.size()<<std::endl;
...
{
	if(TEST)std::cout<<"generate_hypothesis"<<std::endl;
						...
						std::vector<track_ptr>& tracks_in)//TODO CHANGE THE FORM OF DETECTION
{
	...
	if(TEST)std::cout<<"#################### JPDAF START ######################### "<<std::endl;
	...
	if(TEST)std::cout<<"########## JPDAF ############# V\n "<<V<<"\n "<<std::endl;
  	...
  	for(uint i = 0; i < hyp_num; ++i){
      			...
      			if(mea_indicator == 1){
    	  			...
    	  			for(uint notZero = 0; notZero < tracksize; ++notZero){
    		  			if(A_matrix(0, notZero) == 1){
						   ...
						   // if(TEST)std::cout<<"########## JPDAF PREDICT ED ##########\n "<<z_predict<<"\n########## S ########\n "<<S<<"\n########## diff #######\n "<<diff<<"\n"<<std::endl;
    			  			cv::Mat S_cv;
    			  			cv::eigen2cv(S, S_cv);
						    ...
						    if(TEST)std::cout<<"########## JPDAF PREDICT B ########## \n "<<b<<"\n "<<(S).determinant()<<std::endl;
    			 			N = N / sqrt((2*CV_PI*S).determinant())*exp(-b);//正态分布
						    if(TEST)std::cout<<"########## JPDAF PREDICT N ##########\n "<<N<<std::endl;
    		  			}
    	  			}
      			}
		    ...
		    if(TEST)std::cout<<Pr(i)<<std::endl;
  	}
	...
	if(TEST)std::cout<<"#################### JPDAF END #########################"<<std::endl;
    return beta;
}
...
void Tracker::manage_tracks(float& time){
	...
	if(prevDetSize == 0){
		...
		if(TEST)std::cout<<"######## manage track not asso ###############"<<not_associated_.size()<<std::endl;
    	...
    	if(TEST)std::cout<<"########## costMat #####\n"<<costMat<<std::endl;
    	if(TEST)std::cout<<"########## ioucostMat #####\n"<<IoucostMat<<std::endl;
    	...
    	for(uint i = 0; i < rows; ++i){
    		for(uint j = 0; j < cols; ++j){
    			if(assigmentsBin.at<int>(i, j)){
					if(TEST)std::cout<<time<<" "<<pretime<<" "<<(float)(time-pretime)<<std::endl;
					...
					if(TEST)std::cout<<vx<<" "<<vy<<" "<<angle<<std::endl;
...
void Tracker::delete_tracks(){	
	for(int i= tracks_.size()-1; i>=0; --i){
		if(TEST)std::cout<<"############################# delete tracks ###############################"<<tracks_[i]->Age()<<std::endl;
		if(tracks_[i]->GetTrackState() == Track_state_.Delete){//TODO
			tracks_.erase(tracks_.begin()+i);
			if(TEST)std::cout<<"############################# delete tracks ###############################"<<std::endl;
		}
	}
}
...
void Tracker::pruning(Detection&  selected_detections,
	...
	if(TEST)std::cout<<"######## pruning costMat ############### \n"<<costMat<<" \n"<<std::endl;
	AssignmentProblemSolver APS;//匈牙利算法
	...
	if(TEST)std::cout<<"######## pruning not asso ###############"<<not_associated_.size()<<std::endl;
