...
def track_one_seq(seq_id, config):
    ...
    for i in range(len(dataset)):
        ...
        obs, ids = tracker.tracking(objects[:, :7],
                                    features=None,
                                    scores=det_scores,
                                    pose=pose,
                                    timestamp=i)
        ...
        test_path = "/home/lulu/Projects/EcoCar/3D-Multi-Object-Tracker/evaluation/test"
        test_file = os.path.join(test_path, str(seq_id).zfill(4)+'.txt')
...
def write_one_tick(ts,tracking_type,P2,V2C,pose,obs,ids,file):
    ...
    for id, ob in enumerate(obs):
        ...
        print(ob)
        box = register_bbs(np.array([ob]), new_pose)
    ...
    for ob_id in tracks.keys():
        ...
        for frame_id in track.trajectory.keys():
            ...
            if frame_id in frame_first_dict.keys():
                frame_first_dict[frame_id][ob_id] = (
                    np.array(ob.updated_state.T), ob.score)
            else:
                frame_first_dict[frame_id] = {
                    ob_id: (np.array(ob.updated_state.T), ob.score)}
